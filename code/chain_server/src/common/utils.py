...
try:
    ...
    logger.error("Triton Trt LLM conector import failed with error: {e}")
...
def get_llm() -> LangChainLLM:
    ...
    logger.info(f"Using {config.llm.model_engine} as model engine for llm. Model name: {config.llm.model_name}")
    if config.llm.model_engine == "triton-trt-llm":
        ...
        raise Exception("LLM Model Engine not supported")
    elif config.llm.model_engine == "nv-ai-foundation":
        #return ChatNVIDIA(model=settings.llm.model_name)
        raise Exception("LLM Model Engine not supported")
    elif config.llm.model_engine == "openai":
        llm = OpenAI(
            openai_api_key=os.getenv('LLM_API_KEY','EMPTY'),
            base_url=config.llm.server_url,
            model=config.llm.model_name,
            #inference_server_url=config.llm.server_url,
            max_tokens=1024,
            # TODO: check what this means top_k
            #top_k=10,
            top_p=1,
            # TODO: check what this means typical_p
            #typical_p=0.95,
            temperature=config.llm.model_temperature,
            frequency_penalty=1.03,
            streaming=True
        )
        return LangChainLLM(llm=llm)
    else:
        raise RuntimeError("Unable to find any supported Large Language Model server. Supported engines are openai.")
...
def get_embedding_model() -> LangchainEmbedding:
    ...
    if torch.cuda.is_available() and config.embeddings.engine_divice == "gpu":
        logger.info("Using GPU as engine divice for embedding")
        model_kwargs["device"] = "cuda:0"

    logger.info(f"Using {config.embeddings.model_engine} as model engine for embeddings")
    encode_kwargs = {"normalize_embeddings": False}
    if config.embeddings.model_engine == "huggingface":
    ...
    elif config.embeddings.model_engine == "nv-ai-foundation":
        return NVIDIAEmbeddings(model=config.embeddings.model_name, model_type="passage")
    else:
...
def get_milvus_connection():
    return connections.connect(uri=get_config().vector_store.url)

def get_milvus_collection():
    get_milvus_connection()
    return Collection(get_config().vector_store.collection_name)